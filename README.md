...
- `app8.js` を起動すると、共通ヘッダーを使った3つの画面（Home/About/FAQ）を確認できます。
...
## 9コマ目の内容 - アプリケーション変数の基本
- `app9.js` を起動すると、アプリケーション変数を通じて全リクエストで共有されるデータを確認できます。
- 画面テンプレートは `views/app9.ejs` で、`app` オブジェクトを参照してサイト名・共有メッセージ・訪問回数を表示します。
- ブラウザをリロードすると `visitCount` が増加し、アプリケーション変数がサーバー内で共有されていることが分かります。
...
### 動作確認のポイント
- アクセスするたびにカウントが増えていけば、アプリケーションスコープで変数が共有されていると理解できます。
